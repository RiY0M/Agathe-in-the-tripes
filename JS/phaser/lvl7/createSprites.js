...
function setHealthBar(value){   //Fonction qui s'occupe de la barre de vie du boss
    ...
    percent_bar = Phaser.Math.Clamp(value, 0, 21) / 21; //Nb de pv : Ici de 0 -> 21

    const centerX = (config.width - width_bar) / 2;

    graphics.clear();
    graphics.fillStyle(0xff6600);
    graphics.fillRoundedRect(centerX, 550, width_bar, 21, 5);

    if (percent_bar > 0){
        graphics.fillStyle(0xff0000);
        graphics.fillRoundedRect(centerX, 550, width_bar * percent_bar, 21, 5)
    }
}